...
type RestOf<T extends IAgent | NewAgent> = Omit<T, 'createdAt' | 'updatedAt'> & {
...
export class AgentService {
  ...
  protected convertDateFromClient<T extends IAgent | NewAgent | PartialUpdateAgent>(agent: T): RestOf<T> {
    return {
      ...agent,
      createdAt: agent.createdAt?.format(DATE_FORMAT) ?? null,
      updatedAt: agent.updatedAt?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAgent: RestAgent): IAgent {
    return {
      ...restAgent,
      createdAt: restAgent.createdAt ? dayjs(restAgent.createdAt) : undefined,
      updatedAt: restAgent.updatedAt ? dayjs(restAgent.updatedAt) : undefined,
    };
  }
