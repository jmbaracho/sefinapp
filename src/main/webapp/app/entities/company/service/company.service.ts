...
type RestOf<T extends ICompany | NewCompany> = Omit<T, 'createdAt' | 'updatedAt'> & {
...
export class CompanyService {
  ...
  protected convertDateFromClient<T extends ICompany | NewCompany | PartialUpdateCompany>(company: T): RestOf<T> {
    return {
      ...company,
      createdAt: company.createdAt?.format(DATE_FORMAT) ?? null,
      updatedAt: company.updatedAt?.format(DATE_FORMAT) ?? null,
    };
  }
